...
class MosquitoDetector:
    ...
    def inference_with_selection(self, data_loader):
        grad_cam = GradCAM(self.model, layer_name='conv2d_7b', threshold=self.threshold) # conv2d_7b for InceptionResNetV2 (last 2d layer)
        all_samples = []
        all_predictions = []
        all_labels = []
        ...
        with torch.no_grad():
            for samples, labels in data_loader:
                samples = samples.unsqueeze(0) if samples.dim() == 3 else samples
        ...
        remaining_slots = 10 - num_positives
        selected_negatives = negative_indices[torch.randperm(len(negative_indices))[:remaining_slots]]

        selected_indices = torch.cat([selected_positives, selected_negatives])
        selected_indices = selected_indices[torch.randperm(len(selected_indices))]

        # Plotting
        cols = 6
        rows = 5
        # num_images = num_positives + remaining_slots
        ...
        for i in range(rows * 2):
            ...
            prediction = all_predictions[sample_index]
            label = all_labels[sample_index]
            ...
            # Plot regular image
            axs[row, col].imshow(sample[0].numpy(), cmap='gray')
            axs[row, col].set_title(title)
            axs[row, col].axis('off')

            # Plot background subtracted image
            axs[row, col + 1].imshow(sample[1].numpy(), cmap='gray')
            axs[row, col + 1].axis('off')

            # Plot saliency map
            axs[row, col + 2].imshow(saliency_map, cmap='hot', interpolation='nearest')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.inference_folder, f"some_samples_{loader_name}.png"))
        plt.show()
    ...
    def save_metrics_to_excel(self, data_loader):
        ...
        metrics_data = {
        "Metric": ["Accuracy", "Precision", "Recall", "F1-Score", "ROC-AUC/AUROC", "PR-AUC/AUPRC"],
...
experiment_folder = '/home/juval.gutknecht/Mosquito_Detection/Inception-ResNet-v2/runs/0410-1604-InceptionResNetV2'
